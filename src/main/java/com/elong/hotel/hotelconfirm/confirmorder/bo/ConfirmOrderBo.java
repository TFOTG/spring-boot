...
public class ConfirmOrderBo extends CompareEntityBase {
		...
		if (order != null && confirmOrder != null) { // 初始化 "目标数据", 此时订单在已审库
			...
			if (orderStatusChange != null) {
				...
				if (orderStatusChange.getPreStatus().equalsIgnoreCase(ElongOrderStatusEnum.H.getStatus())) {
					...
					ConfirmOrderConfig serviceConfig = ConfigurationManager.getHotSwitchConfig("ConfirmOrderConfig",
							ConfirmOrderConfig.class);
					if (Math.abs(orderStatusChange.getOperatorTime().getTime() / 1000
							- confirmOrder.getAmendTime().getTime() / 1000) > serviceConfig.getChangeMinutes()) {
			...
			this.ratePlanId = order.getRatePlanId();
			this.distance = order.getDistanceFromHotelWhenBooking();
			this.confirmType = getConfirmType(order.getOrderFlag(), order.getOtaType());
	...
	private Integer getConfirmType(long orderFlag, Integer otaType) {
		// 直连确认
